...
export default function GSAFlowV167() {
      ...
      if (session?.user) {
        ...
        if (prof) {
          ...
          const { data: nProf } = await supabase.from('perfis_usuarios').insert({ 
            id: session.user.id, expira_em: dataExp.toISOString(), nome_empresa: 'Gomes Serviços Ágeis',
  ...
  const salvarLancamento = async () => {
    ...
    if (idEmEdicao) {
        await supabase.from('lancamentos').update({ 
            ...
            data_vencimento: novaData, categoria: novaCategoria, user_id: user.id, comprovante_url: novoComprovante || null
        }).eq('id', idEmEdicao);
    } else {
        const novos = [];
        for (let i = 0; i < novoParcelas; i++) {
            ...
            novos.push({
                ...
                categoria: novaCategoria, user_id: user.id, status: 'agendado', comprovante_url: novoComprovante || null
            });
        }
  ...
  if (user && assinaturaVencida) return (
    <div className="min-h-screen bg-[#06080a] flex items-center justify-center p-6 text-white text-center font-sans">
      <div className="bg-zinc-900 border border-red-500/40 p-10 rounded-[3.5rem] w-full max-w-md shadow-2xl backdrop-blur-xl animate-in zoom-in duration-300">
        <div className="text-7xl mb-6">🔒</div>
        <h2 className="text-3xl font-black text-red-500 uppercase italic mb-4 tracking-tighter leading-none">Acesso Suspenso</h2>
        <p className="text-zinc-500 text-sm mb-10 leading-relaxed italic font-medium">Sua licença expirou em {dataExpiracao?.toLocaleDateString('pt-BR')}.</p>
        <div className="space-y-4">
          <a href="https://www.mercadopago.com.br" className="block bg-blue-600 hover:bg-blue-500 text-white p-5 rounded-3xl font-black uppercase text-[11px] tracking-widest shadow-lg transform hover:scale-105 transition-all">Renovar Agora</a>
          <button onClick={() => window.location.reload()} className="text-zinc-600 text-[10px] font-black uppercase underline block w-full tracking-widest hover:text-white transition-colors">Já realizei o pagamento</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#06080a] flex items-center justify-center p-6 text-white text-center font-sans">
      <div className="bg-zinc-900 border border-white/5 p-10 rounded-[3rem] w-full max-w-md shadow-2xl backdrop-blur-xl">
        ...
        <form onSubmit={async (e) => { e.preventDefault(); const { error } = isSignUp ? await supabase.auth.signUp({email, password}) : await supabase.auth.signInWithPassword({email, password}); if (error) alert(error.message); else window.location.reload(); }} className="space-y-4 text-left">
          ...
          <button type="submit" className="w-full bg-blue-600 text-white font-black p-4 rounded-2xl uppercase text-xs tracking-widest shadow-lg shadow-blue-500/20">{isSignUp ? 'Criar Acesso' : 'Entrar no Cockpit'}</button>
        </form>
    ...
    <div className="min-h-screen bg-[#06080a] text-zinc-300 p-4 sm:p-8 font-sans text-sm pb-24 overflow-x-hidden">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-white/10 p-8 rounded-[3rem] w-full max-w-lg shadow-2xl relative">
            ...
            <div className="flex gap-3 mt-10">
              <button onClick={atualizarPerfil} className="flex-1 bg-blue-600 text-white p-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg italic transition-all hover:bg-blue-500">Gravar Alterações</button>
      ...
      {itensAtrasadosGeral.length > 0 && (
        <div className="max-w-7xl mx-auto mb-6 bg-red-600/10 border border-red-500/30 p-4 rounded-3xl flex justify-between items-center gap-4 backdrop-blur-md shadow-lg shadow-red-900/10 animate-in slide-in-from-top duration-500">
           <div className="flex items-center gap-3">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-red-500 font-sans italic">Pendências Críticas: {itensAtrasadosGeral.length} vencidos.</p>
           </div>
           <button onClick={() => setDataVisualizacao(new Date(itensAtrasadosGeral[0].data_vencimento + 'T00:00:00'))} className="bg-red-600 text-white px-4 py-2 rounded-2xl text-[8px] font-black uppercase tracking-widest shadow-lg shadow-red-600/20 transition-all active:scale-95">Resolver Agora</button>
        </div>
      )}
      ...
      <header className="flex flex-col sm:flex-row justify-between items-center mb-10 max-w-7xl mx-auto gap-4">
        ...
        <div className="flex gap-4 items-center bg-zinc-900/40 p-2 px-6 rounded-full border border-white/5 shadow-xl backdrop-blur-md">
            <div className="flex items-center gap-4 text-zinc-500 text-[10px] font-black uppercase tracking-widest font-sans font-bold">
            ...
            <button onClick={() => supabase.auth.signOut().then(() => window.location.reload())} className="text-[9px] font-black uppercase text-zinc-600 hover:text-red-400 font-sans ml-2 transition-colors">Sair</button>
        </div>
      </header>

      {/* CARDS RESUMO */}
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-zinc-900/30 border border-white/5 p-6 rounded-[2.5rem] shadow-xl backdrop-blur-md"><p className="text-blue-500 text-[8px] font-black uppercase mb-1 tracking-widest font-sans font-bold">Entradas</p><h2 className="text-2xl font-black italic text-white">{formatarMoeda(totalReceitas)}</h2></div>
        <div className="bg-zinc-900/30 border border-white/5 p-6 rounded-[2.5rem] shadow-xl backdrop-blur-md"><p className="text-red-500 text-[8px] font-black uppercase mb-1 tracking-widest font-sans font-bold">Saídas</p><h2 className="text-2xl font-black italic text-white">{formatarMoeda(totalDespesas)}</h2></div>
        <div className="bg-zinc-900/30 border border-white/5 p-6 rounded-[2.5rem] shadow-xl backdrop-blur-md"><p className="text-zinc-500 text-[8px] font-black uppercase mb-1 tracking-widest font-sans font-bold">Saldo Previsto</p><h2 className={`text-2xl font-black italic ${lucroLiquido >= 0 ? 'text-green-500' : 'text-red-500'}`}>{formatarMoeda(lucroLiquido)}</h2></div>
        <div className="bg-zinc-900/10 border border-dashed border-white/10 p-6 rounded-[2.5rem] flex items-center justify-center text-center backdrop-blur-sm"><p className="text-zinc-700 text-[8px] font-black uppercase italic tracking-widest font-sans font-bold leading-none">Meta Alvo: <br/>{formatarMoeda(perfil?.meta_faturamento || 0)}</p></div>
      </div>

      {/* TRIPLE COCKPIT INTEGRAL */}
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div className="bg-zinc-900/40 border border-white/5 p-8 rounded-[2.5rem] h-[320px] flex flex-col items-center justify-between shadow-2xl relative overflow-hidden backdrop-blur-md">
            <p className="text-zinc-500 text-[9px] font-black uppercase tracking-widest font-sans font-bold">Battery Status</p>
            ...
            <p className="text-blue-500/40 text-[8px] font-black uppercase tracking-widest italic font-sans font-bold">Cockpit Efficiency</p>
        </div>

        {/* GRÁFICO 2: TENDÊNCIA */}
        <div className="bg-zinc-900/40 border border-white/5 p-6 rounded-[2.5rem] h-[320px] shadow-2xl flex flex-col backdrop-blur-md">
            <p className="text-zinc-500 text-[9px] font-black uppercase tracking-widest mb-6 text-center font-sans font-bold">Trend Analisys (5M)</p>
        ...
        <div className="bg-zinc-900/40 border border-white/5 p-6 rounded-[2.5rem] h-[320px] shadow-2xl flex flex-col items-center overflow-hidden backdrop-blur-md">
            <p className="text-zinc-500 text-[9px] font-black uppercase tracking-widest mb-4 text-center font-sans font-bold">Category Distribution</p>
                    ...
                    <div className="mt-2 flex flex-wrap justify-center gap-x-4 gap-y-1 overflow-y-auto max-h-[80px] no-scrollbar">
                        {gastosPorCategoria.map((entry: any, index: number) => (
                            <div key={entry.name} className="flex items-center gap-2">
                                ...
                                <span className="text-[9px] font-bold uppercase tracking-tight text-zinc-400 leading-none">{entry.name}</span>
                            </div>
                        ))}
                    </div>
                    </>
                ) : ( <div className="h-full flex items-center justify-center text-zinc-800 text-[8px] font-black uppercase tracking-widest italic font-sans leading-none font-bold">No Data Points</div> )}
            </div>
        </div>
      </div>
      ...
      <div className={`max-w-7xl mx-auto p-6 rounded-[3rem] border backdrop-blur-sm mb-10 shadow-2xl transition-all duration-500 ${idEmEdicao ? 'bg-blue-600/5 border-blue-500/50' : 'bg-zinc-900/20 border-white/5'}`}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-4">
          ...
          <select className="bg-black/40 p-3 h-12 rounded-2xl border border-white/5 text-xs text-white font-black uppercase outline-none cursor-pointer" value={novaCategoria} onChange={e => setNovaCategoria(e.target.value)}>
            {perfil?.categorias?.map((cat: string) => <option key={cat} value={cat} className="bg-zinc-900 text-white font-bold">{cat}</option>)}
          </select>
          
          <select className="bg-black/40 p-3 h-12 rounded-2xl border border-white/5 text-xs text-white font-black outline-none cursor-pointer" value={novoParcelas} onChange={e => setNovoParcelas(Number(e.target.value))}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          <input type="text" placeholder="Descrição do lançamento..." className="bg-black/40 p-4 h-14 rounded-2xl border border-white/5 text-xs text-white outline-none focus:border-blue-500/30 font-sans" value={novaDescricao} onChange={e => setNovaDescricao(e.target.value)} />
          ...
          <button onClick={salvarLancamento} className="bg-blue-600 text-white font-black h-14 rounded-2xl text-[10px] uppercase tracking-widest shadow-lg shadow-blue-500/20 hover:scale-[1.01] transition-all italic active:scale-95 leading-none font-sans">Lançar Fluxo</button>
        </div>
      </div>
          ...
          return (
            <div key={item.id} className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 rounded-[2.5rem] border transition-all duration-300 gap-4 shadow-xl ${eConfirmado ? 'bg-black/20 border-white/5 opacity-50' : estaAtrasado ? 'bg-red-500/5 border-red-500/30 animate-pulse-slow' : 'bg-zinc-900/20 border-white/5 hover:bg-zinc-900/40'}`}>
              <div className="flex items-center gap-4 w-full">
                ...
                <div className="truncate"><p className={`font-bold text-base tracking-tight truncate max-w-[200px] sm:max-w-md ${eConfirmado ? 'line-through text-zinc-600' : 'text-zinc-100'}`}>{item.descricao}</p><p className="text-[9px] font-black uppercase text-zinc-600 tracking-widest mt-1 italic font-sans leading-none">{item.categoria}</p></div>
              </div>
              <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto border-t border-white/5 pt-4 sm:pt-0 sm:border-none">
                ...
                <div className="flex gap-4">
                    ...
                    <button onClick={() => { setIdEmEdicao(item.id); setNovaDescricao(item.descricao); setNovoValor(item.valor.toString()); setNovaData(item.data_vencimento); setNovoTipo(item.tipo); setNovaCategoria(item.categoria); setNovoComprovante(item.comprovante_url || ''); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="text-zinc-700 hover:text-white transition-colors text-xl leading-none">✏️</button>
                    <button onClick={async () => { if(confirm('Excluir lançamento?')) { await supabase.from('lancamentos').delete().eq('id', item.id); carregarLancamentos(); } }} className="text-zinc-800 hover:text-red-500 transition-colors text-xl leading-none">🗑️</button>
                </div>
              </div>
            </div>
          );
      ...
      {/* CSS GLOBAL PARA SELECTS E EFEITOS */}
      <style jsx global>{`
