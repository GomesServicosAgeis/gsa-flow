...
export default function GSAFlowV132() {
  ...
  return (
    <div className="min-h-screen bg-[#06080a] text-zinc-300 p-4 sm:p-8 font-sans text-sm pb-24 overflow-x-hidden">
      ...
      {/* CARDS COM FORMATAÇÃO CORRIGIDA */}
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-zinc-900/30 border border-white/5 p-6 rounded-[2.5rem]"><p className="text-blue-500 text-[8px] font-black uppercase mb-1">Entradas</p><h2 className="text-2xl font-black italic text-white">{formatarMoeda(totalReceitas)}</h2></div>
        <div className="bg-zinc-900/30 border border-white/5 p-6 rounded-[2.5rem]"><p className="text-red-500 text-[8px] font-black uppercase mb-1">Saídas</p><h2 className="text-2xl font-black italic text-white">{formatarMoeda(totalDespesas)}</h2></div>
        <div className="bg-zinc-900/30 border border-white/5 p-6 rounded-[2.5rem]"><p className="text-zinc-500 text-[8px] font-black uppercase mb-1">Saldo Previsto</p><h2 className={`text-2xl font-black italic ${lucroLiquido >= 0 ? 'text-green-500' : 'text-red-500'}`}>{formatarMoeda(lucroLiquido)}</h2></div>
        <div className="bg-zinc-900/10 border border-dashed border-white/10 p-6 rounded-[2.5rem] flex items-center justify-center"><p className="text-zinc-700 text-[8px] font-black uppercase italic tracking-widest text-center">Meta Mensal: <br/>{formatarMoeda(perfil.meta_faturamento)}</p></div>
      </div>
                ...
                {gastosPorCategoria.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            ...
                            {/* LEGENDA COM COR CORRIGIDA */}
                            <Legend verticalAlign="bottom" wrapperStyle={{ fontSize: '9px', fontWeight: 'bold', color: '#a1a1aa' }} />
                        </PieChart>
                    </ResponsiveContainer>
                ) : (
      ...
      <div className={`max-w-7xl mx-auto p-6 rounded-[3rem] border backdrop-blur-sm mb-10 shadow-2xl transition-all duration-500 ${idEmEdicao ? 'bg-blue-600/5 border-blue-500/50' : 'bg-zinc-900/20 border-white/5'}`}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-4">
          <div className="flex bg-black/40 rounded-2xl p-1 h-12 border border-white/5">
            <button onClick={() => setNovoTipo('entrada')} className={`flex-1 rounded-xl text-[9px] font-black uppercase ${novoTipo === 'entrada' ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-600'}`}>Receita</button>
            <button onClick={() => setNovoTipo('saida')} className={`flex-1 rounded-xl text-[9px] font-black uppercase ${novoTipo === 'saida' ? 'bg-red-600 text-white shadow-lg' : 'text-zinc-600'}`}>Saída</button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          ...
          <button onClick={salvarLancamento} className="bg-blue-600 text-white font-black h-14 rounded-2xl text-[10px] uppercase tracking-widest shadow-lg shadow-blue-500/20 italic">Lançar no Cockpit</button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-3 mb-20">
        {lancamentosDoMes.map((item) => {
          ...
          return (
            <div key={item.id} className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 rounded-[2.5rem] border transition-all duration-300 gap-4 ${eConfirmado ? 'bg-black/20 border-white/5 opacity-50' : estaAtrasado ? 'bg-red-500/5 border-red-500/30 animate-pulse-slow' : 'bg-zinc-900/20 border-white/5 hover:bg-zinc-900/40'}`}>
              <div className="flex items-center gap-4 w-full sm:w-auto">
                <span className={`text-[10px] font-mono px-4 py-2 rounded-xl font-bold min-w-[60px] text-center ${estaAtrasado ? 'bg-red-500 text-white' : 'bg-zinc-800 text-zinc-500'}`}>{new Date(item.data_vencimento + 'T00:00:00').toLocaleDateString('pt-BR', {day: '2-digit', month: '2-digit'})}</span>
                <div className="truncate">
                  <p className={`font-bold text-base tracking-tight truncate max-w-[200px] sm:max-w-none ${eConfirmado ? 'line-through text-zinc-600' : 'text-zinc-100'}`}>{item.descricao}</p>
