  ...
  if (user && assinaturaVencida) return (
    <div className="min-h-screen bg-[#06080a] flex items-center justify-center p-6 text-white text-center font-sans overflow-hidden">
      <div className="bg-zinc-900 border border-red-500/40 p-10 rounded-[3.5rem] w-full max-w-md shadow-[0_0_60px_rgba(239,68,68,0.2)] backdrop-blur-xl relative z-10 animate-in zoom-in duration-300">
        ...
        <p className="text-zinc-500 text-sm mb-10 leading-relaxed italic font-medium">Sua licença expirou em {dataExpiracao?.toLocaleDateString('pt-BR')}. <br/>Reative para continuar seus lançamentos.</p>
    ...
    <div className="min-h-screen bg-[#06080a] flex items-center justify-center p-6 text-white text-center font-sans">
      <div className="bg-zinc-900 border border-white/5 p-10 rounded-[3rem] w-full max-w-md shadow-2xl backdrop-blur-xl">
        ...
        <form onSubmit={async (e) => { e.preventDefault(); const { error } = isSignUp ? await supabase.auth.signUp({email, password}) : await supabase.auth.signInWithPassword({email, password}); if (error) alert(error.message); else window.location.reload(); }} className="space-y-4 text-left">
          ...
          <button type="submit" className="w-full bg-blue-600 text-white font-black p-4 rounded-2xl uppercase text-xs tracking-widest shadow-lg shadow-blue-600/20">{isSignUp ? 'Criar Acesso' : 'Entrar no Cockpit'}</button>
        </form>
    ...
    <div className="min-h-screen bg-[#06080a] text-zinc-300 p-4 sm:p-8 font-sans text-sm pb-24 overflow-x-hidden">
      ...
      {itensAtrasadosGeral.length > 0 && (
        <div className="max-w-7xl mx-auto mb-6 bg-red-600/10 border border-red-500/30 p-4 rounded-3xl flex justify-between items-center gap-4 backdrop-blur-md shadow-lg shadow-red-900/10 animate-in slide-in-from-top duration-500">
           <div className="flex items-center gap-3">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-red-500 font-sans italic">Pendências Detectadas: {itensAtrasadosGeral.length} itens vencidos.</p>
           </div>
      ...
      <header className="flex flex-col sm:flex-row justify-between items-center mb-10 max-w-7xl mx-auto gap-4">
        <div>
          ...
          <p className="text-[8px] text-zinc-600 font-bold uppercase tracking-[0.4em] mt-1 italic leading-none">{perfil?.nome_empresa}</p>
        </div>
        ...
        <div className="flex gap-4 items-center bg-zinc-900/40 p-2 px-6 rounded-full border border-white/5 shadow-xl backdrop-blur-md">
            ...
            <button onClick={() => supabase.auth.signOut().then(() => window.location.reload())} className="text-[9px] font-black uppercase text-zinc-600 hover:text-red-400 transition-colors font-sans leading-none">Sair</button>
        </div>
      </header>
            ...
            <div key={item.id} className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 rounded-[2.5rem] border transition-all duration-300 gap-4 shadow-xl ${eConfirmado ? 'bg-black/20 border-white/5 opacity-50' : estaAtrasado ? 'bg-red-500/5 border-red-500/30 animate-pulse-slow' : 'bg-zinc-900/20 border-white/5 hover:bg-zinc-900/40'}`}>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto border-t border-white/5 pt-4 sm:pt-0 sm:border-none">
                ...
                <div className="flex gap-4">
                    {!eConfirmado && <button onClick={async () => { await supabase.from('lancamentos').update({ status: 'confirmado' }).eq('id', item.id); carregarLancamentos(); }} className="bg-white text-black text-[9px] font-black px-5 py-2 rounded-full uppercase tracking-widest shadow-xl hover:bg-blue-600 transition-all font-sans leading-none">Pagar</button>}
                    ...
                    <button onClick={async () => { if(confirm('Remover?')) { await supabase.from('lancamentos').delete().eq('id', item.id); carregarLancamentos(); } }} className="text-zinc-800 hover:text-red-500 transition-colors text-xl leading-none">🗑️</button>
                </div>
              </div>
            </div>
          );
