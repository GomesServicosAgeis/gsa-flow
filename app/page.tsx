...
export default function GSAFlowUltimateSaaS() {
  ...
  async function carregarDadosSaaS(userId: string) {
    ...
    if (prof) {
      ...
      const { data: novoProf } = await supabase.from('perfis_usuarios').insert({ 
        id: userId, nome_empresa: 'Meu Negócio', meta_faturamento: 10000, categorias: categoriasPadrao, expira_em: dataExp.toISOString()
      }).select().single();
  ...
  const salvarLancamento = async () => {
    ...
    const payload = { descricao: novaDescricao, valor: Number(novoValor), tipo: novoTipo, data_vencimento: novaData, categoria: novaCategoria, comprovante_url: novoComprovante, user_id: user.id };
  ...
  if (loading) return <div className="min-h-screen bg-[#0b0e14] flex items-center justify-center text-blue-500 font-black animate-pulse uppercase italic text-2xl">GSA FLOW</div>;
    ...
    return (
      <div className="min-h-screen bg-[#0b0e14] flex items-center justify-center p-6 text-white text-center font-sans">
        <div className="bg-zinc-900 border border-red-500/50 p-10 rounded-[3rem] w-full max-w-md shadow-2xl">
          ...
          <h1 className="text-2xl font-black text-red-500 uppercase italic mb-2 tracking-tighter leading-none">Acesso Suspenso</h1>
          <p className="text-zinc-400 text-sm mb-8 leading-relaxed">Olá, <strong>{perfil.nome_empresa}</strong>. Seu acesso expirou. Escolha como deseja renovar:</p>
          <div className="space-y-3">
            <a href="https://www.mercadopago.com.br/subscriptions/checkout?preapproval_plan_id=eb0e8f15cbbd4be085473bca86164037" className="block bg-blue-600 hover:bg-blue-500 text-white font-black p-4 rounded-2xl transition-all uppercase text-[10px] tracking-widest shadow-lg shadow-blue-600/20">Assinar Mensal (Cartão)</a>
            <a href="https://mpago.li/1fcbewH" className="block bg-zinc-800 hover:bg-zinc-700 text-white font-black p-4 rounded-2xl transition-all uppercase text-[10px] tracking-widest border border-zinc-700">Pagar 30 dias (Pix/Boleto)</a>
          </div>
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#0b0e14] flex items-center justify-center p-6 text-white font-sans text-center">
      <div className="bg-zinc-900 border border-zinc-800 p-10 rounded-[3rem] w-full max-w-md shadow-2xl">
        ...
        <form onSubmit={handleAuth} className="space-y-4 text-left">
          <input type="email" placeholder="E-mail" className="w-full bg-zinc-950 p-4 rounded-2xl border border-zinc-800 text-white focus:border-blue-500 outline-none text-sm" value={email} onChange={e => setEmail(e.target.value)} required />
          <input type="password" placeholder="Senha" className="w-full bg-zinc-950 p-4 rounded-2xl border border-zinc-800 text-white focus:border-blue-500 outline-none text-sm" value={password} onChange={e => setPassword(e.target.value)} required />
          <button type="submit" className="w-full bg-blue-600 text-white font-black p-4 rounded-2xl uppercase text-xs tracking-widest shadow-lg shadow-blue-600/20">Entrar no Cockpit</button>
        </form>
        <button onClick={() => setIsSignUp(!isSignUp)} className="w-full mt-8 text-zinc-500 text-[9px] font-black uppercase tracking-widest">{isSignUp ? 'Já tem conta? Login' : 'Registre-se Grátis'}</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#0b0e14] text-white p-4 sm:p-8 font-sans text-sm pb-24">
      {mostrarConfig && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[2.5rem] w-full max-w-lg shadow-2xl">
            <h2 className="text-xl font-black text-blue-500 uppercase italic mb-6 leading-none">Configurações</h2>
            <div className="space-y-4 mb-8 text-left">
              <input type="text" placeholder="Empresa" className="w-full bg-zinc-950 p-3 rounded-xl border border-zinc-800 text-white outline-none" value={editNomeEmpresa} onChange={e => setEditNomeEmpresa(e.target.value)} />
              ...
              <div className="flex gap-2">
                <input type="text" placeholder="Categoria" className="flex-1 bg-zinc-950 p-2 rounded-xl border border-zinc-800 text-white outline-none" value={novaCatInput} onChange={e => setNovaCatInput(e.target.value)} />
      ...
      {itensAtrasadosGeral.length > 0 && (
        <div className="max-w-6xl mx-auto mb-6 bg-red-600/10 border border-red-500/30 p-4 rounded-2xl flex justify-between items-center gap-4 animate-pulse-slow">
           <p className="text-xs font-bold text-white">⚠️ Olá {perfil.nome_empresa}, você tem {itensAtrasadosGeral.length} contas pendentes.</p>
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-[2rem] h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart margin={{ top: 0, bottom: 20, left: 0, right: 0 }}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: '#18181b', border: 'none', borderRadius: '12px', fontSize: '10px', color: '#fff' }} itemStyle={{ color: '#fff' }} />
                  <Legend verticalAlign="bottom" wrapperStyle={{ color: '#a1a1aa', fontSize: '9px', fontWeight: 'bold', paddingTop: '10px' }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto bg-zinc-900/20 border border-zinc-800 p-8 rounded-[2.5rem] mb-10 h-[400px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={dadosAnuais} margin={{ top: 10, bottom: 10, left: 10, right: 10 }}>
              ...
              <Tooltip cursor={{fill: '#27272a'}} contentStyle={{ backgroundColor: '#18181b', border: 'none', borderRadius: '12px', fontSize: '10px', color: '#fff' }} itemStyle={{ color: '#fff' }} />
              <Legend wrapperStyle={{ color: '#a1a1aa', fontSize: '10px', paddingTop: '10px' }} />
      ...
      <div className={`max-w-6xl mx-auto p-6 rounded-[2.5rem] border mb-10 transition-all ${idEmEdicao ? 'bg-blue-600/10 border-blue-500' : 'bg-zinc-900/40 border-zinc-800/50'}`}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
          ...
          <select className="bg-zinc-950 p-3 h-12 rounded-xl border border-zinc-800 text-xs text-white uppercase font-black outline-none" value={novaRecorrencia} onChange={e => setNovaRecorrencia(e.target.value)} disabled={!!idEmEdicao}><option value="unico">Único</option><option value="mensal">Mensal</option></select>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
          ...
          <input type="text" placeholder="Link/Notas" className="bg-zinc-950 p-4 h-12 rounded-xl border border-zinc-800 text-xs text-white outline-none" value={novoComprovante} onChange={e => setNovoComprovante(e.target.value)} />
          <button onClick={salvarLancamento} className="bg-blue-600 hover:bg-blue-500 text-white font-black h-12 rounded-xl text-[9px] uppercase tracking-widest transition-all shadow-lg shadow-blue-600/20">{idEmEdicao ? 'Salvar' : 'Lançar'}</button>
        </div>
      </div>
              ...
              <div className="flex items-center gap-4 w-full sm:w-auto">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <p className={`font-bold truncate max-w-[200px] text-white ${eConfirmado ? 'line-through text-zinc-600' : ''}`}>{item.descricao}</p>
