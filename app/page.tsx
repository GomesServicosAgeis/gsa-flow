...
export default function GSAFlowV165() {
  ...
  useEffect(() => {
    const sessionInit = async () => {
      ...
      if (session?.user) {
        // Busca perfil com prioridade absoluta
        ...
        if (prof) {
          ...
          // Fallback: Cria perfil se não existir (Trial de 3 dias)
  ...
  const salvarLancamento = async () => {
    ...
    setNovaDescricao(''); setNovoValor(''); setIdEmEdicao(null); setNovoParcelas(1); setNovoComprovante(''); 
    carregarLancamentos();
  };
  ...
  const itensAtrasadosGeral = lancamentos.filter(i => 
    i.status !== 'confirmado' && new Date(i.data_vencimento + 'T00:00:00') < hoje
  );
  ...
  if (user && assinaturaVencida) return (
    <div className="min-h-screen bg-[#06080a] flex items-center justify-center p-6 text-white text-center font-sans">
      <div className="bg-zinc-900 border border-red-500/40 p-10 rounded-[3.5rem] w-full max-w-md shadow-2xl backdrop-blur-xl animate-in zoom-in duration-300">
        ...
        <h2 className="text-3xl font-black text-red-500 uppercase italic mb-4">Acesso Expirado</h2>
        <p className="text-zinc-500 text-sm mb-10 leading-relaxed italic font-medium">
          Sua licença expirou em {dataExpiracao?.toLocaleDateString('pt-BR')}.
        </p>
        <div className="space-y-4">
          <a href="https://www.mercadopago.com.br" className="block bg-blue-600 hover:bg-blue-500 text-white p-5 rounded-3xl font-black uppercase text-[11px] tracking-widest shadow-lg shadow-blue-600/30 transition-all transform hover:scale-105">Renovar Agora</a>
    ...
    <div className="min-h-screen bg-[#06080a] text-zinc-300 p-4 sm:p-8 font-sans text-sm pb-24 overflow-x-hidden">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto no-scrollbar">
                  {perfil?.categorias?.map((cat: string) => (
                    <span key={cat} className="bg-zinc-800/50 border border-white/5 px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase">{cat} <button onClick={() => setPerfil({...perfil, categorias: perfil.categorias.filter((c: string) => c !== cat)})} className="text-red-500 ml-1 hover:text-white">×</button></span>
                  ))}
                </div>
              </div>
            </div>
            <div className="flex gap-3 mt-10">
              <button onClick={atualizarPerfil} className="flex-1 bg-blue-600 text-white p-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg italic">Gravar Alterações</button>
      ...
      {itensAtrasadosGeral.length > 0 && (
        <div className="max-w-7xl mx-auto mb-6 bg-red-600/10 border border-red-500/30 p-4 rounded-3xl flex justify-between items-center gap-4 backdrop-blur-md shadow-lg shadow-red-900/10">
           <div className="flex items-center gap-3">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-red-500 font-sans italic leading-none">Pendências Detectadas: {itensAtrasadosGeral.length} itens vencidos.</p>
           </div>
           <button onClick={() => setDataVisualizacao(new Date(itensAtrasadosGeral[0].data_vencimento + 'T00:00:00'))} className="bg-red-600 text-white px-4 py-2 rounded-2xl text-[8px] font-black uppercase tracking-widest shadow-lg shadow-red-600/20">Resolver Agora</button>
        </div>
      )}
      ...
      <header className="flex flex-col sm:flex-row justify-between items-center mb-10 max-w-7xl mx-auto gap-4">
        ...
        <div className="flex gap-4 items-center bg-zinc-900/40 p-2 px-6 rounded-full border border-white/5 shadow-xl backdrop-blur-md">
            ...
            <button onClick={() => supabase.auth.signOut().then(() => window.location.reload())} className="text-[9px] font-black uppercase text-zinc-600 hover:text-red-400 font-sans leading-none ml-2">Sair</button>
        </div>
      </header>

      {/* CARDS DE VALORES INTEGRAIS */}
      ...
      {/* TRIPLE COCKPIT INTEGRAL (GRÁFICOS COMPLETOS) */}
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div className="bg-zinc-900/40 border border-white/5 p-8 rounded-[2.5rem] h-[320px] flex flex-col items-center justify-between shadow-2xl relative overflow-hidden backdrop-blur-md">
            ...
            <p className="text-blue-500/40 text-[8px] font-black uppercase tracking-widest italic tracking-[0.2em] font-sans">Cockpit Efficiency</p>
        </div>
        ...
        {/* PIZZA DE CATEGORIAS (LEGENDA MANUAL) */}
        <div className="bg-zinc-900/40 border border-white/5 p-6 rounded-[2.5rem] h-[320px] shadow-2xl flex flex-col items-center overflow-hidden backdrop-blur-md">
            ...
            <div className="flex-1 w-full relative">
                {gastosPorCategoria.length > 0 ? (
                    ...
                    <div className="h-full flex items-center justify-center text-zinc-800 text-[8px] font-black uppercase tracking-widest italic font-sans leading-none">Sem dados registrados</div>
                )}
            </div>
        </div>
      </div>

      {/* FORMULÁRIO DE LANÇAMENTO INTEGRAL */}
      <div className={`max-w-7xl mx-auto p-6 rounded-[3rem] border backdrop-blur-sm mb-10 shadow-2xl transition-all duration-500 ${idEmEdicao ? 'bg-blue-600/5 border-blue-500/50' : 'bg-zinc-900/20 border-white/5'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          ...
          <button onClick={salvarLancamento} className="bg-blue-600 text-white font-black h-14 rounded-2xl text-[10px] uppercase tracking-widest shadow-lg shadow-blue-500/20 hover:scale-[1.01] transition-all italic active:scale-95 leading-none font-sans">Lançar no Cockpit</button>
        </div>
      </div>

      {/* LISTAGEM DE LANÇAMENTOS INTEGRAL */}
            ...
            <div key={item.id} className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 rounded-[2.5rem] border transition-all duration-300 gap-4 shadow-xl ${eConfirmado ? 'bg-black/20 border-white/5 opacity-50' : estaAtrasado ? 'bg-red-500/5 border-red-500/30 animate-pulse-slow' : 'bg-zinc-900/20 border-white/5 hover:bg-zinc-900/40'}`}>
              <div className="flex items-center gap-4 w-full">
                ...
                <div className="truncate">
                  ...
                  <p className="text-[9px] font-black uppercase text-zinc-600 tracking-widest mt-1 italic font-sans leading-none">{item.categoria}</p>
                </div>
              </div>
